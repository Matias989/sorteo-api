...
mongoose.connect('mongodb+srv://raziel:155524400@sorteo.haag8qf.mongodb.net/?retryWrites=true&w=majority&appName=sorteo')
  .then(() => {
...
app.post('/api/draw', async (req, res) => {
    try {
      ...
      if (participants.length === 0) {
        return res.status(400).json({ message: 'No hay usuarios registrados para el sorteo.' });
      }
      ...
      await Promise.all(participants.map(async user => {
        ...
        user.history.push({ date: new Date(), result: esGanador ? 'win' : 'lose' });
  
        // Multiplicador
        const mult = parseFloat(user.multiplier.toString());
        user.multiplier = esGanador ? Math.max(1, mult * 0.5) : Math.min(10, mult + 0.5);
